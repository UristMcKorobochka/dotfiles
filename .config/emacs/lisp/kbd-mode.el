...
;; HACK HACK HACK
(defun kbd-movement-by-symbol-in-layer-definitions ()
  "Use `forward-symbol' instead of `forward-word' on M-f.
Same for `backward-word'"
  (progn (let ((start) (end))
	   (while (not (eq (point) (point-max)))
	     (progn
	       (re-search-forward "(def\\(s\\|l\\)" nil 1)
	       (setq start (match-beginning 0))
	       (re-search-forward "^)" nil 1)
	       (setq end (match-beginning 0))
	       ;; why don't working
	       ;;	       (with-silent-modifications (add-text-properties start end `(keymap ,@kbd-layer-definition-map syntax-table ,@kbd-layer-definition-syntax-table))))))
	       (put-text-property start end 'keymap kbd-layer-definition-map)
	       (put-text-property start end 'syntax-table kbd-layer-definition-syntax-table))))
	       (goto-char 0)))

;;; Vars
  (defvar kbd-layer-definition-syntax-table
    (let ((stf (make-syntax-table)))
      ;; We want punctuatuion inside a layer definition to be a symbols between which we can move
      (modify-syntax-entry ?\( "_" stf)
      (modify-syntax-entry ?\) "_" stf)
      (modify-syntax-entry ?\[ "_" stf)
      (modify-syntax-entry ?\] "_" stf)
      (modify-syntax-entry ?\{ "_" stf)
      ...
      ;; We want
      stf)
    "The syntax table used inside layer defenitions")
  
(defvar kbd-layer-definition-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map emacs-lisp-mode-map)
    (define-key map "\M-f" 'forward-symbol)
    (define-key map "\M-b" (lambda () "like `forward-symbol' but backwards" (interactive) (forward-symbol -1)))
    map))
...
;; Make 'M-f/b' work on symbols when they're first-class citizens
(add-hook 'kbd-mode-hook 'kbd-movement-by-symbol-in-layer-definitions)
